...
class PermissionsController < ApplicationController
  ...
  def create
    ...
    if @permission.save
      redirect_to redirect_save(permissions_path, new_permission_path()), notice: "#{I18n.t (:notice_permission_successful_created)}"
    else
      render action: 'new'
    end
  end
  ...
  def update
    ...
    if @permission.update_attributes(params[:permission])
      redirect_to redirect_save(permissions_path, edit_permission_path(@permission)), notice: "#{I18n.t (:notice_function_successful_updated)}"
    else
      render action: 'edit'
    end
  end
