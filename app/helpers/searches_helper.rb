...
module SearchesHelper
  def display_link(res, params)
    ...
    link_to(raw("#{highlight(result, params)}"), "/#{String::keep_clean_space(res.class.to_s.underscore.pluralize)}/#{res.id}/edit", :class => "search_result", :style => "font-size:12px; color: #467aa7;")
  end

  def display_description(res, params=[])
    if defined?  res.description
      highlight(res.description, params)
    end
  end
