  ...
  def index
    ...
    def save_uos
      ...
      params[:input_id].keys.each do |r|
        Input.where(id: params[:input_id]["#{r}"].to_i).delete_all
        #if input.nil?
          input = Input.new
        ...
        #  input.update_attribute("gross_high", params[:gross_high]["#{r}"])
    ...
    def load_gross
      ...
      @result[:"gross_low_#{@index.to_s}"] = params[:size_low].to_i * abacus_value * weight.to_f * productivity_ratio.to_f
